package com.example.android.quakereportFinal;
...
public final class QueryUtils {
     ...
     */
    public static ArrayList<Location> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<Location> earthquakes = new ArrayList<>();

        // Try to parse the SAMPLE_JSON_RESPONSE. If there's a problem with the way the JSON
        // is formatted, a JSONException exception object will be thrown.
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
        try {
            JSONObject eJson = new JSONObject(SAMPLE_JSON_RESPONSE);
            ...
            for (int i = 0; i < 10; i++) {
                ...
                earthquakes.add(new Location(Float.parseFloat(magnitude), place, Long.parseLong(date), url));
        ...
        Log.v("list", String.valueOf(earthquakes.size()));
